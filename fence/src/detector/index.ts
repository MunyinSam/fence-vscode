...
import { getCurrentUserEmail, getMyCommittedFiles, getMyLineNumbers } from '../git';
 ...
 */
export async function detect(projectPath: string, singleFile?: string): Promise<Skill[]> {

    // ── Resolve file list ──────────────────────────────────────────────────
    let codeFiles: string[]; // paths relative to projectPath

    if (singleFile) {
        const rel = path.relative(projectPath, singleFile).replace(/\\/g, '/');
        if (isIgnored(rel) || !SUPPORTED.has(ext(singleFile))) { return []; }
        codeFiles = [rel];
    } else {
        const allEntries = await fs.readdir(projectPath, { recursive: true });
        codeFiles = (allEntries as string[]).filter(f =>
    ...
    let myCommittedFiles: Set<string> = new Set();
    ...
    const astBreadth: Record<string, number> = {}; // skill → files with ≥1 hit
        ...
        for (const f of relevantFiles) {
            ...
            for (let pi = 0; pi < regexes.length; pi++) {
                ...
                for (let li = 0; li < fileLines.length; li++) {
                    ...
                    if (myLines !== null && !myLines.has(li + 1)) { continue; }
                    re.lastIndex = 0;
