...
async function detect(projectPath) {
  ...
  const SUPPORTED = /* @__PURE__ */ new Set([".ts", ".js", ".tsx", ".jsx", ".mjs", ".cjs", ".py", ".go", ".java", ".cs", ".rs", ".rb", ".php"]);
  const codeFiles = allEntries.filter(
  ...
  const TS_EXTS = /* @__PURE__ */ new Set([".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"]);
...
`;
  ...
  const known = skills.filter((s) => s.level === 0 /* Knows */);
  const learning = skills.filter((s) => s.level === 1 /* Learning */);
...
program2.command("init").description("Scan project and create CLAUDE.md").argument("[path]", "path to project (defaults to current directory)").action(async (projectPath = process.cwd()) => {
  const detected = await detect(projectPath);
  await saveSkills(detected);
  const allSkills = await LoadSkills();
  const content = generate(allSkills);
